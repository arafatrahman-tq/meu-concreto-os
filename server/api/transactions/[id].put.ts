...
import { checkAndUpdateSaleStatus } from '../../utils/sales'
...
export default defineEventHandler(async (event) => {
  ...
  try {
    const updatedTransaction = await db.transaction(async (tx) => {
      ...
      // Se a transação foi marcada como paga e tem venda vinculada, verifica se a venda deve ser completada
      if (updated.status === 'paid') {
        let saleIdToCheck: number | null = null

        if (updated.saleId) {
          // Transação diretamente vinculada à venda
          saleIdToCheck = updated.saleId
        } else if (updated.parentTransactionId) {
          // É uma parcela - busca o saleId da transação pai
          const parentTransaction = await tx
            .select({ saleId: transactions.saleId })
            .from(transactions)
            .where(eq(transactions.id, updated.parentTransactionId))
            .get()

          if (parentTransaction?.saleId) {
            saleIdToCheck = parentTransaction.saleId
          }
        }

        if (saleIdToCheck) {
          await checkAndUpdateSaleStatus(saleIdToCheck, tx)
        }
      }
