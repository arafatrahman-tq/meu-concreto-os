...
export default defineEventHandler(async (event) => {
  ...
  for (const q of allQuotes) {
    ...
    const quoteStatus = normalizeQuoteStatus(q.status);
    ...
    processEntry(
      ...
      },
      {
        ...
        totalQuoted: q.total,
        pendingQuotes:
          quoteStatus === "draft" || quoteStatus === "negotiation" ? 1 : 0,
        approvedQuotes: quoteStatus === "approved" ? 1 : 0,
      },
    );
  }
  ...
  for (const s of allSales) {
    ...
    const saleStatus = normalizeSaleStatus(s.status);
    ...
    processEntry(
      ...
      {
        salesCount: 1,
        completedSales: ["completed", "billed"].includes(saleStatus) ? 1 : 0,
        cancelledSales: saleStatus === "cancelled" ? 1 : 0,
        totalSpent: ["completed", "billed"].includes(saleStatus) ? s.total : 0,
      },
    );
  }
