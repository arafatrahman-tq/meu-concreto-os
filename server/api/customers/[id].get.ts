...
export default defineEventHandler(async (event) => {
    ...
    const [customerQuotes, customerSales] = await Promise.all([
        db.select().from(quotes).where(eq(quotes.customerName, customer.name)).all(),
    ...
    return {
        customer: {
            ...
            stats: {
                ...
                totalSpent: customerSales.reduce((acc, s) => acc + (s.status === 'completed' ? s.total : 0), 0)
            }
        }
    }
})
