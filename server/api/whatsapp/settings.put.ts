...
export default defineEventHandler(async (event) => {
  ...
  // Server-side validation for enabled features
  if (data.alertsEnabled && (!data.alertRecipients || data.alertRecipients.length === 0)) {
    throw createError({ statusCode: 400, message: 'Alertas requerem pelo menos um destinatário.' })
  }
  if (data.reportsEnabled && (!data.reportRecipients || data.reportRecipients.length === 0)) {
    throw createError({ statusCode: 400, message: 'Relatórios requerem pelo menos um destinatário.' })
  }
  if (data.schedulesReminderEnabled && (!data.schedulesReminderRecipients || data.schedulesReminderRecipients.length === 0)) {
    throw createError({ statusCode: 400, message: 'Lembretes de agendamento requerem pelo menos um destinatário.' })
  }
