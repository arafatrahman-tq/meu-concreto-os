...
export default defineEventHandler(async (event) => {
  ...
  if (mode === 'ping') {
    ...
    return {
      ...
      message: result.ok
        ? 'API do WhatsApp acessível.'
        : `Erro ao acessar API (HTTP ${result.status}).`
    }
  }
  ...
  if (!toNumber || !/^\+\d{5,15}$/.test(toNumber)) {
    throw createError({
      statusCode: 400,
      message: 'toNumber inválido. Use formato: +5511999999999'
    })
  }
  ...
  if (!config.phoneNumber) {
    throw createError({
      statusCode: 400,
      message: 'Número de WhatsApp não configurado. Conecte primeiro.'
    })
  }

  const now = formatInAppTime(new Date(), 'dd/MM/yyyy \'às\' HH:mm', ptBR)
