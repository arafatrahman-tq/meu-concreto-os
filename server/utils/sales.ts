...
export async function checkAndUpdateSaleStatus(
  ...
  if (allPaid) {
    ...
    // Só atualiza se a venda não estiver já completed ou cancelled ou billed
    if (
      currentSale &&
      !["completed", "billed", "cancelled"].includes(currentSale.status)
    ) {
      // If it has transactions, maybe we should set to 'billed' instead of 'completed'?
      // But the script logic here is specifically for auto-completing.
      // If we are auto-completing because of financial reconciliation, 'billed' might be appropriate.
      await client
        .update(sales)
        .set({
          status: "billed",
