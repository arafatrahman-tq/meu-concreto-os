...
export async function generateDocumentPDF(
  ...
  doc.text(splitCompanyName, 15, 52)

  doc.setFontSize(8)
  doc.setFont('helvetica', 'normal')
  doc.setTextColor(113, 113, 122) // zinc-500
  let compY = 52 + splitCompanyName.length * 4
  ...
  doc.setFontSize(9)
  doc.setTextColor(24, 24, 27) // zinc-900
  const splitCustomerName = doc.splitTextToSize(
    data.customerName.toUpperCase(),
    colWidth
  )
  doc.text(splitCustomerName, 15 + colWidth * 2, 52)

  doc.setFontSize(8)
  doc.setFont('helvetica', 'normal')
  doc.setTextColor(113, 113, 122) // zinc-500
  let cliY = 52 + splitCustomerName.length * 4
  ...
  // 7.5 Validity Message (Large/Bold Highlight)
  if (data.validityMessage) {
    ...
    doc.setFont('helvetica', 'bold')
    doc.setTextColor(34, 197, 94) // primary-500
    const splitValidityMsg = doc.splitTextToSize(
      data.validityMessage.toUpperCase(),
      pageWidth - 30
    )
    doc.text(splitValidityMsg, pageWidth / 2, currentY, { align: 'center' })
    currentY += splitValidityMsg.length * 7
  }
