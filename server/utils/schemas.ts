...
const optionalIntField = z.preprocess((val) => {
...
const optionalSlumpIntField = z.preprocess((val) => {
  if (val === "" || val === null || val === undefined) return undefined;
  if (typeof val === "string") {
    const normalized = val.trim().replace("±", "+-").replace(/\s+/g, "");

    const toleranceMatch = normalized.match(/^(\d+)\+-(\d+)$/);
    if (toleranceMatch) {
      return Number(toleranceMatch[1]);
    }
...
const optionalNullableSlumpIntField = z.preprocess((val) => {
...
export const productSchema = z.object({
  ...
  fck: optionalIntField, // MPa
  slump: optionalSlumpIntField, // cm (aceita "10+-2")
  stoneSize: z.string().optional(), // e.g., "brita 0"
  mixDesignId: z.preprocess((val) => {
    ...
    return val;
  }, z.number().int().optional().nullable()), // Link to production recipe
  active: z.boolean().default(true),
});
...
export const quoteItemSchema = z.object({
  ...
  quantity: z.number().gt(0, { message: "A quantidade deve ser positiva" }),
...
export const quoteSchema = z.object({
  ...
  validityMessage: z.string().optional().nullable(),
