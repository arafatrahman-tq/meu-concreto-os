...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  try {
    ...
    if (status === 401) {
      formRef.value.setErrors([{ name: 'password', message: 'E-mail ou senha incorretos.' }])
    } else if (status === 403) {
      formRef.value.setErrors([{ name: 'email', message: 'Acesso negado: dispositivo não autorizado.' }])
    } else if (status === 400) {
      formRef.value.setErrors([{ name: 'email', message: 'Preencha todos os campos corretamente.' }])
    } else {
      formRef.value.setErrors([{ name: 'email', message: 'Erro ao conectar ao servidor. Tente novamente.' }])
    }
  } finally {
    loading.value = false
  }
        ...
        >
          ...
          >
            ...
            <UFormField
              ...
              name="email"
              eager-validation
              :ui="{ label: 'text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400' }"
            >
              <UInput
                v-model="state.email"
                type="email"
            ...
            >
              <UInput
                ...
                :ui="{ base: 'rounded-2xl', trailingIcon: 'cursor-pointer hover:text-primary-500 transition-colors' }"
                class="w-full"
                :disabled="loading"
            ...
            <div class="flex items-center justify-between py-1">
              <UCheckbox
                ...
                :disabled="loading"
                :ui="{
                  label: 'text-[10px] font-black text-zinc-400 uppercase tracking-widest cursor-pointer',
            ...
            <UButton
              ...
              :ui="{ base: 'rounded-2xl py-4 font-black uppercase tracking-[0.2em]' }"
            >
