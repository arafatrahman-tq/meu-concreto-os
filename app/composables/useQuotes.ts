...
export const useQuotes = () => {
  ...
  const filteredQuotes = computed(() => {
    return quotes.value
      .filter((q) => {
        ...
        const targetDate = q.date || q.createdAt;
        const txDate = parseItemDate(targetDate);
  ...
  const exportFilteredCsv = () => {
    ...
    a.href = url;
    a.download = `orcamentos-filtrados-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  const exportFilteredExcel = async () => {
    ...
    writeFile(workbook, `orcamentos-${formatISODate(new Date())}.xlsx`);
  };
  ...
  const exportFilteredPdf = async () => {
    ...
    const periodLabel = (() => {
      ...
      if (rows.length > 0) {
        const dates = rows
          .map((q) => parseItemDate(q.date || q.createdAt)?.getTime())
    ...
    doc.save(`orcamentos-${formatISODate(new Date())}.pdf`);
    toast.add({
      title: "PDF gerado",
