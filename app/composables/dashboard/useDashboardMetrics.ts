...
export const useDashboardMetrics = (
    ...
    const toMs = (v: string | number | Date | null | undefined): number => {
        if (!v) return 0
        const n = typeof v === 'number' ? (v < 1e10 ? v * 1000 : v) : new Date(v).getTime()
        return n
    }
    ...
    const salesFiltered = computed(() =>
        data.sales.value.filter(
            s => toMs(s.date) >= ranges.value.start && toMs(s.date) < ranges.value.end && s.status !== 'cancelled'
        )
    )
    const salesPrevious = computed(() =>
        data.sales.value.filter(
            s => toMs(s.date) >= ranges.value.prevStart && toMs(s.date) < ranges.value.prevEnd && s.status !== 'cancelled'
        )
    )
    ...
    const revenueTrend = computed(() => {
        ...
        return ((revenueFiltered.value - revenuePrevious.value) / revenuePrevious.value) * 100
    })
    ...
    const quotesFiltered = computed(() =>
        data.quotes.value.filter(q => toMs(q.date) >= ranges.value.start && toMs(q.date) < ranges.value.end)
    )
    const quotesPrevious = computed(() =>
        data.quotes.value.filter(q => toMs(q.date) >= ranges.value.prevStart && toMs(q.date) < ranges.value.prevEnd)
    )
    const quotesTrend = computed(() => {
        if (selectedPeriod.value === 'all' || !quotesPrevious.value.length) return null
        return ((quotesFiltered.value.length - quotesPrevious.value.length) / quotesPrevious.value.length) * 100
    })

    // ─── Sales Count Trend ───────────────────────────────────────
    const salesCountTrend = computed(() => {
        if (selectedPeriod.value === 'all' || !salesPrevious.value.length) return null
        return ((salesFiltered.value.length - salesPrevious.value.length) / salesPrevious.value.length) * 100
    })

    const trendLabel = computed(() => {
        if (selectedPeriod.value === 'all' || selectedPeriod.value === 'custom') return ''
    ...
    const txFiltered = computed(() =>
        data.transactions.value.filter(
            t => toMs(t.date) >= ranges.value.start && toMs(t.date) < ranges.value.end && t.status !== 'cancelled'
        )
    )
    ...
    const balanceFiltered = computed(() => incomeFiltered.value - expenseFiltered.value)
    ...
    const sparklineRevenue = computed(() => {
        ...
        if (selectedPeriod.value === 'today') {
            ...
            return Array.from({ length: 24 }, (_, h) => {
                ...
                return data.sales.value
                    .filter(s => toMs(s.date) >= start && toMs(s.date) < end && s.status !== 'cancelled')
        ...
        if (selectedPeriod.value === 'week' || selectedPeriod.value === 'month') {
            ...
            return Array.from({ length: days }, (_, i) => {
                ...
                return data.sales.value
                    .filter(s => toMs(s.date) >= start && toMs(s.date) < start + 86_400_000 && s.status !== 'cancelled')
                    .reduce((a, s) => a + (s.total ?? 0), 0)
            })
        }
        ...
        if (selectedPeriod.value === 'custom' && customStart.value && customEnd.value) {
            ...
            const endMs = new Date(`${customEnd.value}T00:00:00`).getTime() + 86_400_000
            ...
            return Array.from({ length: count }, (_, i) => {
                ...
                return data.sales.value
                    .filter(s => toMs(s.date) >= bStart && toMs(s.date) < bEnd && s.status !== 'cancelled')
                    .reduce((a, s) => a + (s.total ?? 0), 0)
            })
        }

        return Array.from({ length: 12 }, (_, i) => {
            const start = new Date(now.getFullYear(), now.getMonth() - (11 - i), 1).getTime()
            const end = new Date(now.getFullYear(), now.getMonth() - (10 - i), 1).getTime()
            return data.sales.value
                .filter(s => toMs(s.date) >= start && toMs(s.date) < end && s.status !== 'cancelled')
                .reduce((a, s) => a + (s.total ?? 0), 0)
        })
    ...
    const cashFlowChartData = computed(() => {
        const now = new Date()
        type Bar = { label: string, income: number, expense: number, revenue: number }

        const createBar = (start: number, end: number, label: string): Bar => ({
            label,
            income: data.transactions.value
                .filter(t => t.type === 'income' && toMs(t.date) >= start && toMs(t.date) < end && t.status !== 'cancelled')
                .reduce((a, t) => a + t.amount, 0),
            expense: data.transactions.value
                .filter(t => t.type === 'expense' && toMs(t.date) >= start && toMs(t.date) < end && t.status !== 'cancelled')
                .reduce((a, t) => a + t.amount, 0),
        ...
        if (selectedPeriod.value === 'week') {
            ...
            return Array.from({ length: 7 }, (_, i) => {
                ...
                return createBar(d.getTime(), d.getTime() + 86_400_000,
        ...
        if (selectedPeriod.value === 'year') {
            return Array.from({ length: 12 }, (_, i) => {
                ...
                return createBar(d.getTime(), new Date(now.getFullYear(), i + 1, 1).getTime(),
        ...
        if (selectedPeriod.value === 'custom' && customStart.value && customEnd.value) {
            ...
            const rangeEndMs = new Date(`${customEnd.value}T00:00:00`).getTime() + 86_400_000
            const dayDiff = Math.ceil((rangeEndMs - rangeStart.getTime()) / 86_400_000)
            ...
            while (cursor.getTime() < rangeEndMs) {
                ...
                bars.push(createBar(mStart, mEnd,
        ...
        return Array.from({ length: 6 }, (_, i) => {
            ...
            return createBar(d.getTime(), new Date(d.getFullYear(), d.getMonth() + 1, 1).getTime(),
                d.toLocaleDateString('pt-BR', { month: 'short' }))
        })
    })
    ...
    const cashFlowSubtitle = computed(() => {
        ...
        if (selectedPeriod.value === 'custom' && customStart.value && customEnd.value) {
            const fmt = (s: string) => new Date(`${s}T00:00:00`).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })
    ...
    const chartMax = computed(() => Math.max(...cashFlowChartData.value.flatMap(m => [m.income, m.expense, m.revenue]), 1))

    // ─── Quick stats ──────────────────────────────────────────────────
    const pendingSales = computed(() => salesFiltered.value.filter(s => s.status === 'pending').length)
    const activeProductsCount = computed(() => data.products.value.filter(p => p.active).length)

    // ─── Listas recentes ──────────────────────────────────────────────
    const recentTransactions = computed(() => [...txFiltered.value].sort((a, b) => toMs(b.date) - toMs(a.date)).slice(0, 6))
    const recentSales = computed(() => [...salesFiltered.value].sort((a, b) => toMs(b.date) - toMs(a.date)).slice(0, 5))

    // ─── Top Vendedores ───────────────────────────────────────────────
    const topSellers = computed(() => {
        const sellerMap = new Map<number, { name: string, total: number, count: number }>()
        salesFiltered.value.forEach((s) => {
            if (!s.seller) return
            const current = sellerMap.get(s.seller.id) || { name: s.seller.name, total: 0, count: 0 }
