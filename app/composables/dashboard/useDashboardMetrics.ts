...
import { normalizeQuoteStatus, normalizeSaleStatus } from "~/utils/status";
...
export const useDashboardMetrics = (
  ...
  const concreteVolumeTrend = computed(() => {
    if (selectedPeriod.value === "all" || !concreteVolumePrevious.value)
      return null;
    return (
      ((concreteVolumeFiltered.value - concreteVolumePrevious.value) /
        concreteVolumePrevious.value) *
      100
    );
  });

  const sparklineVolume = computed(() => {
    const now = new Date();

    const getVolumeForRange = (start: number, end: number) => {
    ...
    if (selectedPeriod.value === "today") {
      ...
      return Array.from({ length: 24 }, (_, h) => {
        ...
        return getVolumeForRange(start, end);
      });
    }
    ...
    if (selectedPeriod.value === "week" || selectedPeriod.value === "month") {
      ...
      return Array.from({ length: days }, (_, i) => {
        ...
        return getVolumeForRange(start, start + 86_400_000);
      });
    }
    ...
    if (
      ...
      return Array.from({ length: count }, (_, i) => {
        ...
        return getVolumeForRange(bStart, bEnd);
      });
    }
  ...
  const pendingSales = computed(
    () =>
      salesFiltered.value.filter(
        (s) => normalizeSaleStatus(s.status) === "open",
      ).length,
  );
  ...
  const topSellers = computed(() => {
    const sellerMap = new Map<
      number,
      { name: string; total: number; count: number; volume: number }
    >();
  ...
  const eventTimeline = computed<DashboardEvent[]>(() => {
    ...
    const saleEvents: DashboardEvent[] = salesAllInRange.value.map((sale) => ({
      ...
      amount: sale.total ?? 0,
      amountTone:
        normalizeSaleStatus(sale.status) === "cancelled"
          ? "neutral"
          : "positive",
      tag:
        normalizeSaleStatus(sale.status) === "cancelled"
          ? "Venda cancelada"
          : ["completed", "billed"].includes(normalizeSaleStatus(sale.status))
            ? "Venda concluída"
            : "Venda em andamento",
    }));
