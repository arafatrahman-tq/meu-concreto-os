...
export const useDashboardFilters = () => {
  ...
  const ranges = computed<DashboardRanges>(() => {
    ...
    if (selectedPeriod.value === "custom") {
      const start = customStart.value
        ? (parseDateInputLocal(customStart.value)?.getTime() ?? 0)
        : 0;
      const end = customEnd.value
        ? (parseDateInputLocal(customEnd.value, {
            endOfDay: true,
          })?.getTime() ?? tomorrow)
        : tomorrow;
  ...
  const fetchWindowStart = computed<string | null>(() => {
    ...
    if (selectedPeriod.value === "custom" && customStart.value) {
      const customDate = parseDateInputLocal(customStart.value);
      const customMs = customDate ? customDate.getTime() : now.getTime();
