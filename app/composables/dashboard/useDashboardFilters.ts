...
export const useDashboardFilters = () => {
    ...
    const ranges = computed<DashboardRanges>(() => {
        ...
        if (selectedPeriod.value === 'today') {
            return { start: today, end: tomorrow, prevStart: yesterday, prevEnd: today }
        }
        ...
        if (selectedPeriod.value === 'week') {
            ...
            return { start, end: addDays(monday, 7).getTime(), prevStart, prevEnd: start }
        }
        ...
        if (selectedPeriod.value === 'custom') {
            ...
            const end = customEnd.value
                ? addDays(startOfDay(new Date(`${customEnd.value}T00:00:00`)), 1).getTime()
                : tomorrow
    ...
    const fetchWindowStart = computed<string | null>(() => {
        ...
        if (selectedPeriod.value === 'custom' && customStart.value) {
            const customDate = new Date(`${customStart.value}T00:00:00`)
