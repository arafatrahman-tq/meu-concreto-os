...
import { formatDateInputPtBR } from "~/utils/date-input";
...
export const useSales = () => {
  ...
  const statusConfig: Record<string, { label: string }> = {
    open: { label: "Aberta" },
    pending: { label: "Aberta" },
    confirmed: { label: "Pendente" },
    in_progress: { label: "Pendente" },
      ...
      .filter((s) => {
        ...
        if (rawDate) {
          ...
          const endDate = dateEnd.value
            ...
            : null;
          if (startDate) matchDate = matchDate && itemDate >= startDate;
          if (endDate) matchDate = matchDate && itemDate <= endDate;
        }
  ...
  const exportFilteredCsv = () => {
    ...
    const lines = filteredSales.value.map((s) => {
      ...
      const statusLabel =
        statusConfig[s.status as string]?.label ||
    ...
    a.href = url;
    a.download = `vendas-${getRangeSlug()}-${new Date().toISOString().slice(0, 10)}.csv`;
  ...
  const exportFilteredExcel = async () => {
    ...
    writeFile(
      workbook,
      `vendas-${getRangeSlug()}-${formatISODate(new Date())}.xlsx`,
    );
  };
  ...
  const exportFilteredPdf = async () => {
    ...
    doc.save(`vendas-${getRangeSlug()}-${formatISODate(new Date())}.pdf`);
    toast.add({
      title: "PDF gerado",
