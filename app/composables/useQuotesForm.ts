...
export const useQuotesForm = (options: UseQuotesFormOptions) => {
  ...
  const form = reactive({
    ...
    paymentMethod2: '',
    notes: '',
    validityMessage: 'PROPOSTA VÁLIDA POR APENAS DOIS DIAS ÚTEIS.',
  ...
  const openEdit = (s: Quote) => {
    ...
    form.notes = s.notes ?? ''
    form.validityMessage = s.validityMessage ?? ''
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        validityMessage: form.validityMessage || null,
      ...
      const restrictedEditPayload = {
        ...
        validityMessage: payload.validityMessage,
        notes: payload.notes
      }
