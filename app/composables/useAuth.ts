  ...
  const refreshAuth = async () => {
    try {
      ...
      if (!stillValid) {
        ...
        if (userData?.defaultCompanyId && data.companies.some(c => c.id === userData.defaultCompanyId)) {
          activeCompanyId.value = userData.defaultCompanyId
        } else {
      ...
      console.error('[useAuth] Failed to refresh auth data:', error)
      return false
    }
  }
