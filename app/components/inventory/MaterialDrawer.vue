...
<template>
  ...
  <BaseSimpleDrawer
    ...
    @save="saveMaterial"
  >
    <form id="material-form" class="flex flex-col gap-8" @submit.prevent="saveMaterial">
      <!-- Seção: Identificação -->
      <BaseDrawerSection title="Identificação" icon="i-heroicons-cube">
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
          ...
          <UFormField label="Custo Unitário (R$)" required :error="formErrors.cost">
            <UInput
          ...
          <UFormField label="Estoque Atual" required :error="formErrors.stock">
            <UInput
              ...
              size="lg"
            >
              <template #trailing>{{ form.unit }}</template>
            </UInput>
          </UFormField>
        </div>
      </BaseDrawerSection>
        ...
        <UFormField label="Status do Insumo" class="col-span-full">
          ...
          >
            <div class="flex items-center gap-3">
              ...
              >
                <UIcon name="i-heroicons-check-circle" class="w-4 h-4 text-primary-500" />
              </div>
              <div>
