...
const validate = (): boolean => {
  ...
  if (waForm.phoneNumber) {
    const digits = waForm.phoneNumber.replace(/\D/g, "");
    if (digits.length < 10 || digits.length > 15) {
      toast.add({
        title: "Número inválido",
        description:
          "O número do WhatsApp deve ter entre 10 e 15 dígitos (incluindo DDI e DDD).",
        color: "error",
  ...
  if (waForm.alertsEnabled && waForm.alertRecipients.length === 0) {
    toast.add({
      title: "Atenção",
      description: "Adicione pelo menos um destinatário para os alertas.",
      color: "warning",
      icon: "i-heroicons-exclamation-triangle",
    });
    isValid = false;
  ...
  if (waForm.reportsEnabled && waForm.reportRecipients.length === 0) {
    toast.add({
      title: "Atenção",
      description: "Adicione pelo menos um destinatário para os relatórios.",
      color: "warning",
      icon: "i-heroicons-exclamation-triangle",
    });
    isValid = false;
  ...
  if (
    waForm.schedulesReminderEnabled &&
    waForm.schedulesReminderRecipients.length === 0
  ) {
    toast.add({
      title: "Atenção",
      description:
        "Adicione pelo menos um destinatário para os lembretes de agendamento.",
      color: "warning",
      icon: "i-heroicons-exclamation-triangle",
    });
    isValid = false;
        ...
        <div class="space-y-3">
          ...
          >
            <div v-if="waForm.alertsEnabled" class="space-y-3 overflow-hidden">
              <div class="px-3 pt-2">
                <PhoneList
                  v-model="waForm.alertRecipients"
                  label="Destinatários dos Alertas"
                  description="Números de WhatsApp que receberão os alertas"
                />
              </div>
            </div>
          </Transition>
          ...
          >
            <div v-if="waForm.reportsEnabled" class="space-y-4 overflow-hidden">
              <div class="px-3 pt-2 space-y-4">
                ...
                <PhoneList
                  v-model="waForm.reportRecipients"
                  label="Destinatários dos Relatórios"
                  description="Números de WhatsApp que receberão o relatório"
                />
          ...
          >
            ...
            >
              <div class="px-3 pt-2">
                ...
                <PhoneList
                  v-model="waForm.schedulesReminderRecipients"
                  label="Destinatários dos Agendamentos"
                  description="Adicione números para receber lembretes"
                />
              </div>
            </div>
          </Transition>
