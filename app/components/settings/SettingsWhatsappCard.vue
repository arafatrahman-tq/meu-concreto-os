...
watch(waSettings, (s) => {
    ...
    waForm.schedulesReminderLeadTimeHours = s.schedulesReminderLeadTimeHours ?? 24;
    waForm.schedulesReminderRecipients = [...(s.schedulesReminderRecipients ?? [])];
    ...
    try {
        await $fetch(`/api/whatsapp/settings?companyId=${props.companyId}`, {
            ...
            body: {
                ...
                schedulesReminderLeadTimeHours: Number(waForm.schedulesReminderLeadTimeHours),
                schedulesReminderRecipients: waForm.schedulesReminderRecipients.filter(Boolean),
...
const handleConnect = async () => {
    ...
    try {
        ...
        toast.add({
            ...
            icon: res.ok ? "i-heroicons-check-circle" : "i-heroicons-exclamation-triangle",
        });
        await refreshWA();
    } catch (e: unknown) {
        const err = e as { data?: { message?: string }; message?: string };
...
const handleDisconnect = async () => {
    ...
    try {
        await $fetch(`/api/whatsapp/disconnect?companyId=${props.companyId}`, { method: "POST" });
        toast.add({
            title: "Desconectado",
        ...
        toast.add({
            title: "Erro",
            description: err?.data?.message ?? err?.message ?? "Falha ao desconectar.",
            color: "error",
            icon: "i-heroicons-exclamation-circle",
        });
    } finally {
...
const handleTestPing = async () => {
    ...
    try {
        ...
        toast.add({
            ...
            icon: res.ok ? "i-heroicons-check-circle" : "i-heroicons-exclamation-circle",
        });
    } catch (e: unknown) {
        const err = e as { data?: { message?: string }; message?: string };
        toast.add({
...
const handleSendReport = async () => {
    ...
    try {
        ...
        toast.add({
            title: "Erro",
            description: err?.data?.message ?? err?.message ?? "Falha ao enviar relatório.",
            color: "error",
            icon: "i-heroicons-exclamation-circle",
        });
    } finally {
    ...
    <UCard>
        <template #header>
            <div class="flex items-center gap-3">
                <div class="w-9 h-9 rounded-xl bg-green-50 dark:bg-green-500/10 flex items-center justify-center shrink-0">
                    <UIcon name="i-simple-icons-whatsapp" class="w-5 h-5 text-green-500" />
                </div>
                <div>
                    ...
                    <div class="flex items-center gap-2 mt-0.5">
                        ...
                        <UBadge v-if="isUsingGlobal" color="primary" variant="soft" size="sm" class="text-[9px] px-1.5 py-0">
                            Global
                        </UBadge>
                    </div>
                </div>
                <UBadge :color="waSettings?.isConnected ? 'success' : 'neutral'" variant="soft" size="sm"
        ...
        <div class="space-y-6">
            ...
            <div v-if="user?.role === 'admin'" class="p-4 rounded-xl border transition-colors duration-200"
                :class="waForm.isGlobal ? 'bg-primary-50 border-primary-200 dark:bg-primary-900/10 dark:border-primary-800/30' : 'bg-zinc-50 border-zinc-200 dark:bg-zinc-800/50 dark:border-zinc-700'">
                <div class="flex items-center justify-between gap-4">
                    <div class="space-y-1">
                        ...
                        <p class="text-xs text-zinc-500 dark:text-zinc-400 max-w-md">
                            Define esta empresa como a provedora oficial da conexão WhatsApp para todo o sistema.
                            <span v-if="waForm.isGlobal"
            ...
            <div v-if="isUsingGlobal && !waForm.isGlobal"
                class="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-800/30">
                <div class="flex items-center justify-between gap-4">
                    <div class="flex items-start gap-3">
                        <div class="p-2 rounded-lg bg-blue-100 dark:bg-blue-800/30 text-blue-600 dark:text-blue-300">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-blue-700/80 dark:text-blue-300/70 mt-1 max-w-sm">
                                Em vez de conectar um WhatsApp próprio, utilize a conexão da Instância Global configurada
                                pelo administrador.
                            </p>
                        </div>
                    </div>
                ...
                <Transition enter-active-class="transition-all duration-300 ease-out"
                    ...
                    <div v-if="!waForm.useGlobal" class="mt-3 pt-3 border-t border-blue-200 dark:border-blue-800/30">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 transition-opacity"
                :class="{ 'opacity-40 pointer-events-none': waForm.useGlobal }">
                <UFormField label="URL da API" class="md:col-span-2">
                    <UInput v-model="waForm.apiUrl" placeholder="http://localhost:3025" icon="i-heroicons-globe-alt"
                        class="w-full" :disabled="user?.role !== 'admin'" />
                </UFormField>

                <UFormField label="API Key">
                    <div class="flex gap-2">
                        <UInput v-model="waForm.apiKey" :type="showApiKey ? 'text' : 'password'"
                            placeholder="Sua chave de API" icon="i-heroicons-key" class="w-full"
                            :disabled="user?.role !== 'admin'" />
                        <UButton color="neutral" variant="ghost" :disabled="user?.role !== 'admin'"
                ...
                <UFormField label="Número Remetente (WhatsApp)">
                    <UInput v-model="waForm.phoneNumber" placeholder="+5511999999999" icon="i-heroicons-phone"
                        class="w-full" :disabled="user?.role !== 'admin'" />
                </UFormField>
            </div>

            <!-- Connection controls -->
            <div v-if="user?.role === 'admin' && !waForm.useGlobal" class="flex flex-wrap gap-2 pt-1">
                <UButton color="success" variant="soft" icon="i-simple-icons-whatsapp" :loading="loadingConnect"
                    @click="handleConnect">
                    Conectar
                </UButton>
                <UButton color="error" variant="soft" icon="i-heroicons-power" :loading="loadingDisconnect"
                    :disabled="!waSettings?.isConnected" @click="handleDisconnect">
                    Desconectar
                </UButton>
                <UButton color="neutral" variant="ghost" icon="i-heroicons-wifi" :loading="loadingTestPing"
                    ...
                    <Transition enter-active-class="transition-all duration-300 ease-out"
                        ...
                        <div v-if="waForm.alertsEnabled" class="space-y-3 overflow-hidden">
                            <div class="px-3 pt-2">
                                ...
                                <div class="space-y-2">
                                    <div v-for="(_, i) in waForm.alertRecipients" :key="i" class="flex items-center gap-2">
                                        <UInput v-model="waForm.alertRecipients[i]" icon="i-heroicons-phone"
                                            placeholder="+55 (00) 00000-0000" class="flex-1" size="sm" />
                                        <UButton color="error" variant="ghost" size="sm" icon="i-heroicons-trash"
                                            @click="waForm.alertRecipients.splice(i, 1)" />
                                    </div>
                                    <UButton color="neutral" variant="ghost" size="sm" icon="i-heroicons-plus"
                        ...
                        <div v-if="waForm.reportsEnabled" class="space-y-4 overflow-hidden">
                            <div class="px-3 pt-2 space-y-4">
                                <UFormField label="Frequência do Envio">
                                    <USelect v-model="waForm.reportSchedule" :items="REPORT_SCHEDULE_OPTS"
                                        value-key="value" label-key="label" class="w-full" size="sm" />
                                </UFormField>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div v-for="(_, i) in waForm.reportRecipients" :key="i" class="flex items-center gap-2">
                                        <UInput v-model="waForm.reportRecipients[i]" icon="i-heroicons-phone"
                                            placeholder="+55 (00) 00000-0000" class="flex-1" size="sm" />
                                        <UButton color="error" variant="ghost" size="sm" icon="i-heroicons-trash"
                                            @click="waForm.reportRecipients.splice(i, 1)" />
                                    </div>
                                    <UButton color="neutral" variant="ghost" icon="i-heroicons-plus" size="sm"
                                        class="w-full justify-center border-2 border-dashed border-zinc-200 dark:border-zinc-800 rounded-xl py-2"
                                ...
                                <UButton color="success" variant="soft" icon="i-heroicons-paper-airplane"
                                    :loading="loadingReport" class="w-full" @click="handleSendReport">
            ...
            <div class="space-y-4">
                ...
                <div class="space-y-3">
                    <div class="flex items-center justify-between gap-4 p-3 rounded-xl bg-zinc-50 dark:bg-zinc-800/50">
                        ...
                        <USwitch v-model="waForm.schedulesReminderEnabled" color="success" />
                    </div>
                    ...
                    <Transition enter-active-class="transition-all duration-300 ease-out"
                        ...
                        leave-from-class="opacity-100 translate-y-0 max-h-[800px]"
                        leave-to-class="opacity-0 -translate-y-2 max-h-0">
                        <div v-if="waForm.schedulesReminderEnabled" class="space-y-3 overflow-hidden">
                            <div class="px-3 pt-2">
                                <div class="flex items-center justify-between gap-4 mb-4">
                                    ...
                                    <UInput v-model="waForm.schedulesReminderLeadTimeHours" type="number" size="sm"
                                        class="w-24" suffix="horas" />
                                </div>
                                ...
                                <div class="space-y-2">
                                    <div v-for="(_, i) in waForm.schedulesReminderRecipients" :key="i"
                                        class="flex items-center gap-2">
                                        <UInput v-model="waForm.schedulesReminderRecipients[i]" icon="i-heroicons-phone"
                                            placeholder="+55 (00) 00000-0000" class="flex-1" size="sm" />
            ...
            <div class="space-y-4">
                ...
                <div class="flex gap-2">
                    <UInput v-model="waTestNum" placeholder="+55 (00) 00000-0000"
                        icon="i-heroicons-chat-bubble-left-right" size="sm" class="flex-1" />
                    <UButton color="success" size="sm" icon="i-heroicons-paper-airplane" :loading="loadingTestMsg"
            ...
            <div class="flex justify-end pt-4 border-t border-zinc-100 dark:border-zinc-800">
                <UButton color="primary" icon="i-heroicons-check" size="md" class="px-6 shadow-lg shadow-primary-500/20"
                    :loading="loadingSave" @click="handleSave">
