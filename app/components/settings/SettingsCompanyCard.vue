...
import { maskPhone, maskCep } from "~/utils/formatters";
import { fetchAddressByCep } from "~/utils/location";
...
let cepTimeout: NodeJS.Timeout;

// Logic: Prevent race conditions in CEP lookup
let lastCepLookup = "";
...
const lookupCep = async () => {
  ...
  try {
    ...
    // Check if the user changed the input while we were waiting
    if (lastCepLookup !== digits) return;
