...
<template>
  <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-6">
    <template v-if="pending">
      <USkeleton v-for="i in 5" :key="i" class="h-40 rounded-2xl" />
    </template>
    <template v-else>
      <DashboardKpiCard
      ...
      <DashboardKpiCard
        ...
        :value="metrics.concreteVolumeFiltered.value.toFixed(1) + ' m³'"
        icon="i-lucide-anvil"
        :trend="metrics.concreteVolumeTrend.value"
        :sparkline="metrics.sparklineVolume.value"
      ...
      <DashboardKpiCard
        ...
        :trend="null"
        :icon-color="
          metrics.balanceFiltered.value >= 0 ? 'text-green-500' : 'text-red-500'
        "
        :icon-bg="
