...
import { formatPhone, maskPhone, maskDocument, maskCep } from '~/utils/formatters'
...
watch(
  () => props.open,
  (open) => {
    if (open && !props.customer) resetForm()
  }
)
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        >
          <UInput
            ...
            size="lg"
            class="w-full"
            @update:model-value="(v: string) => (form.document = maskDocument(v))"
          />
        </UFormField>
        ...
        >
          <UInput
            ...
            size="lg"
            class="w-full"
            @update:model-value="(v: string) => (form.phone = maskPhone(v))"
          />
        </UFormField>
      </div>
      ...
      <div class="grid grid-cols-2 gap-5">
        <UFormField label="CEP">
          <UInput
            ...
            class="w-full"
            :loading="loadingCep"
            @update:model-value="(v: string) => (form.cep = maskCep(v))"
            @blur="fetchCep"
          />
        </UFormField>
      </div>
