...
//   - status is not 'cancelled' (no one can delete cancelled sales from the UI)
//   - if 'completed', only admins can delete
          ...
          >
            ...
            <td class="px-4 py-3.5 text-right">
              ...
              >
                ...
                <!-- Quick Actions: Confirm Orders -->
                <UTooltip
                  v-if="s.status === 'confirmed'"
                  text="Iniciar Entrega (Em Progresso)"
                >
                  <UButton
                    color="warning"
                    variant="ghost"
                    icon="i-heroicons-truck"
                    size="xs"
                    @click="emit('updateStatus', s, 'in_progress')"
                  />
                </UTooltip>
                <UTooltip
                  v-if="s.status === 'in_progress'"
                  text="Finalizar e Concluir Venda"
                >
                  <UButton
                    color="success"
                    variant="ghost"
                    icon="i-heroicons-check-badge"
                    size="xs"
                    @click="emit('updateStatus', s, 'completed')"
                  />
                </UTooltip>
                <UTooltip
                  :text="canCancel(s) ? 'Cancelar Venda' : 'Venda já cancelada'"
                >
                  <UButton
                    color="warning"
                    variant="ghost"
                    ...
                    @click="canCancel(s) && emit('cancel', s)"
                  />
                </UTooltip>
                <UTooltip :text="deleteTooltip(s)">
                  <UButton
                    color="error"
                    variant="ghost"
                    ...
                    @click="canDelete(s) && emit('delete', s)"
                  />
                </UTooltip>
              </div>
            </td>
