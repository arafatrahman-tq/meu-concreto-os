...
/**
 * Handle company switch with confirmation toast
 * All data refreshes automatically via reactivity (watch: [companyId])
 */
const handleSwitchCompany = (companyId: number, companyName: string) => {
  // Don't switch if already on this company
  if (companyId === activeCompany.value?.id) return

  // Update the company in state - this triggers reactivity across all composables
  switchCompany(companyId)

  // Show success toast
  toast.add({
    title: 'Empresa alterada',
    description: `Você está agora em: ${companyName}`,
    color: 'success',
...
const companySwitcherItems = computed<DropdownMenuItem[]>(() => {
  ...
  return sorted.map(c => ({
    ...
    onSelect: () => handleSwitchCompany(c.id, c.name)
  }))
})
</script>
