...
const kpiItems = computed(() => [
  ...
  },
  {
    ...
    suffix: props.kpis.balance >= 0 ? 'em conta (positivo)' : 'em conta (negativo)',
    icon: 'i-heroicons-banknotes',
    color: props.kpis.balance >= 0 ? 'text-primary-500' : 'text-red-500',
    bg: props.kpis.balance >= 0 ? 'bg-primary-50 dark:bg-primary-500/10' : 'bg-red-50 dark:bg-red-500/10',
  },
  {
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
    ...
    >
      <div class="flex items-center justify-between gap-2">
        ...
        <div :class="[kpi.bg, 'w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 shadow-xs']">
          <UIcon :name="kpi.icon" :class="['w-6 h-6', kpi.color]" />
        </div>
      </div>
      <p class="text-3xl font-black tabular-nums tracking-tighter" :class="kpi.color.includes('primary') ? 'text-primary-500' : kpi.color">
        {{ kpi.value }}
      </p>
