...
- **Financeiro**: transações, formas de pagamento, fluxo de caixa, parcelamentos
- **Logística**: agendamentos de entrega
- **Notificações**: sistema interno de notificações + integração com WhatsApp
- **Acesso Mobile**: PIN de acesso rápido para interface mobile otimizada
...
- `HWID_SALT`: string opcional mas RECOMENDADA para temperar hashes do Hardware ID (lock de sessão por browser)

**Opcionais:**

- `CRON_SECRET`: token para autenticação de tarefas agendadas (WhatsApp reminders/reports)
...
```
app/
├── app.config.ts          # Configuração do Nuxt UI (cores, variantes)
├── app.vue                # Root component com splash screen de carregamento
├── assets/css/main.css    # Tailwind + tema customizado (fonte Bahnschrift)
├── components/            # Componentes por domínio
│   ├── base/              # Componentes base reutilizáveis (UI System)
│   │   ├── BaseConfirmModal.vue      # Modal de confirmação genérico
│   │   ├── BaseDeleteModal.vue       # Modal especializado em exclusão
│   │   ├── BaseEntityActionModal.vue # Modal para ações em entidades (driver/pumper)
│   │   ├── BaseDrawer.vue            # Drawer base com estrutura padronizada
│   │   ├── BaseSimpleDrawer.vue      # Drawer simplificado para formulários básicos
│   │   ├── BaseDrawerSection.vue     # Seção de drawer com header estilizado
│   │   └── BaseFormField.vue         # Wrapper para campos de formulário
│   ├── companies/         # CompanySwitcher, CompanyDrawer
│   ├── customers/         # CustomerDrawer
│   ├── dashboard/         # KPIs, gráficos, cards (ChartCashFlow, KpiGrid, etc.)
│   ├── inventory/         # MaterialDrawer (insumos)
│   ├── mix-designs/       # MixDesignDrawer (traços)
...
│   ├── payment-methods/   # PaymentMethodDrawer
│   ├── products/          # ProductDrawer
│   ├── settings/          # Cards de configurações (conta, empresa, WhatsApp, etc.)
│   ├── NavigationMenu.vue # Menu lateral de navegação
│   ├── NotificationPopover.vue # Notificações do sistema
│   └── ...
...
│   ├── useNotifications.ts
│   ├── useLogistics.ts
│   ├── usePromoteQuote.ts # Converter orçamento em venda
│   └── dashboard/         # Composables do dashboard (useDashboardData, etc.)
├── layouts/
...
│   ├── produtos.vue       # Cadastro de produtos
│   ├── orcamentos/        # Gestão de orçamentos
│   ├── vendas/            # Gestão de vendas
│   ├── transacoes.vue     # Controle financeiro
│   ├── agendamentos.vue   # Calendário de entregas
│   ├── mobile/            # Interface mobile otimizada (PIN + agendamento)
│   └── ...
├── plugins/
...
│   ├── companies/         # CRUD de empresas + pdf-notes
│   ├── customers/         # CRUD de clientes
│   ├── products/          # CRUD de produtos
│   ├── quotes/[id]/       # download.get.ts, send-pdf.post.ts
│   ├── sales/[id]/        # bill.post.ts, download.get.ts, send-pdf.post.ts
│   ├── transactions/      # CRUD + installments.post.ts (parcelamento)
│   ├── schedules/         # Agendamentos + send-whatsapp.post.ts
│   ├── whatsapp/          # Integração com WhatsApp (Baileys)
│   └── setup-admin.get.ts # Endpoint de setup inicial
├── database/
│   ├── schema.ts          # Schema Drizzle completo (~912 linhas)
│   └── migrations/        # Arquivos SQL de migração
├── middleware/
│   └── auth.ts            # Validação de sessão para /api/*
├── types.d.ts             # Augmentação do H3EventContext (auth)
└── utils/
...
- Login via PIN (4-6 dígitos) por empresa
- Código único da empresa para acesso rápido
- Sessão especial marcada com `isQuickAccess: true`

**Troca de Empresa (CompanySwitcher):**

- Ao trocar de empresa, todos os dados são atualizados em tempo real via reatividade
- Composables e páginas usam `watch: [companyId]` para reagir às mudanças
- Toast de confirmação é exibido: "Empresa alterada - Você está agora em: [Nome]"
- Implementado em: `app/components/CompanySwitcher.vue`

**Páginas com watch: [companyId]:**

- `pages/index.vue` (dashboard) - via `useDashboardData`
- `pages/vendas/index.vue` - via `useSales`
- `pages/transacoes.vue`
- `pages/clientes.vue` - via função reativa `() => /api/customers?companyId=${companyId.value}`
- `pages/configuracoes.vue` - via função reativa
- `pages/agendamentos.vue` - via `query: computed(() => ({ companyId: companyId.value }))`
- `pages/formas-de-pagamento.vue`
- `pages/insumos.vue`
- `pages/vendedores.vue`
- `pages/usuarios.vue`
- `pages/tracos.vue`
- `pages/produtos.vue`

**Composables com watch: [companyId]:**

- `composables/useSales.ts`
- `composables/useQuotes.ts`
- `composables/useLogistics.ts`
- `composables/useNotifications.ts`

**Componentes com watch: [companyId]:**

- `components/transactions/TransactionDrawer.vue`
- `components/transactions/TransactionInstallmentsModal.vue`
...
| `userAuth`         | Credenciais (password hash, HWID, último login) |
...
| `schedules`        | Agendamentos de entrega/serviço                 |
| `materials`        | Insumos para traços                             |
| `mixDesigns`       | Traços/receitas de concreto                     |
| `mixDesignItems`   | Composição dos traços                           |
| `notifications`    | Notificações internas por empresa               |
| `whatsappSettings` | Configurações de WhatsApp por empresa           |
...
- Datas sem hora são parseadas como meio-dia UTC para evitar deslocamento de dia
...
- Usuários podem pertencer a múltiplas empresas via `userCompanies`
...
bun run scripts/seed-demo.ts    # Dados de demonstração completos
...
- **Mensagens de erro e UI**: Português brasileiro (pt-BR)
...
- Aspas simples para strings em TypeScript, duplas em templates Vue
...
- Campos de data aceitam string vazia e null via `dateSchema`
...
- Fonte principal: Bahnschrift (Windows) / DIN Alternate (fallback)
- Use `UButton`, `UInput`, `UCard`, `USlideover`, `UModal`, etc.
- `UApp` é obrigatório no root (app.vue)

### 7.7 Componentes Base (UI System)

Localizados em `app/components/base/`, seguem princípios UI/UX:

**Princípios Aplicados:**

- **DRY (Don't Repeat Yourself)**: Código comum extraído em componentes reutilizáveis
- **Consistência Visual**: Spacing, cores, tipografia uniformes em toda a aplicação
- **8-point Grid**: `p-6`, `gap-4`, `gap-5`, `h-12` (48px touch targets)
- **Miller's Law**: Formulários divididos em seções lógicas (BaseDrawerSection)
- **Hick's Law**: Redução de escolhas desnecessárias via componentes padronizados

**Componentes Disponíveis:**

| Componente              | Props Principais                           | Uso                             |
| ----------------------- | ------------------------------------------ | ------------------------------- |
| `BaseConfirmModal`      | title, confirmLabel, confirmColor, loading | Modais de confirmação           |
| `BaseDeleteModal`       | target, targetLabel, displayValue          | Exclusão com preview            |
| `BaseEntityActionModal` | type, action, entityName                   | Ações em driver/pumper          |
| `BaseDrawer`            | title, isEditing, size, saveLabel          | Drawer genérico                 |
| `BaseSimpleDrawer`      | entityName, isEditing                      | Drawer para formulários simples |
| `BaseDrawerSection`     | title, icon, variant                       | Seções de formulário            |
| `BaseFormField`         | label, required, error, help               | Campos de formulário            |

**Exemplo de Uso (Refatoração):**
...
- Redução de ~40% no código de drawers
- Consistência garantida em todos os formulários
...
- Endpoints dedicados: `/api/quotes/[id]/download.get.ts`, `/api/sales/[id]/download.get.ts`
- Observações personalizadas por empresa via `companies.pdfNotes`
...
- Comando: `wget -qO- --header="Authorization: Bearer <token>" http://localhost:3000/api/whatsapp/process-reminders`

**Relatórios automáticos:**

- Schedule: `0 8 * * *` (todos os dias às 08:00)
- Comando: `wget -qO- --header="Authorization: Bearer <token>" http://localhost:3000/api/whatsapp/process-reports`
...
- Lembretes de agendamento
- Notificações para vendedores e clientes
...
11. **Estado global** - para autenticação, use `useAuth()` composable
12. **Data parsing** - use `parseDate()` do `server/utils/db.ts` para normalizar datas
