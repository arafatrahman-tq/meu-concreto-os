...
## ⚠️ Importante: autenticação dos endpoints de cron

Os endpoints abaixo são protegidos por token Bearer e só executam com `CRON_SECRET` válido:
...
1. `server/middleware/auth.ts` (exceção apenas para esses 2 endpoints com Bearer válido).
2. `server/api/whatsapp/process-reminders.get.ts` (validação explícita do token).
3. `server/api/whatsapp/process-reports.get.ts` (validação explícita do token).
...
Como os endpoints de cron podem disparar mensagens e consumir créditos da API, o token é obrigatório.
...
1. O mesmo padrão é aplicado em `server/api/whatsapp/process-reports.get.ts`.
1. O middleware já permite somente esses endpoints quando o Bearer token for válido.
